...
function render() {
  ...
  if (state.activeNav==="students") {
    ...
    const ab=el("button",{className:"btn-primary"},t("addStudent")); ab.onclick=()=>showAddStudentModal(); actions.appendChild(ab);
  }
  renderPage();
}
function renderPage() {
...
function showStudentProfile(st) {
  ...
  const footer=el("div",{className:"modal-footer"}); const editBtn=el("button",{className:"btn-primary"},t("editInfo")); editBtn.onclick=()=>showEditStudentModal(st); footer.appendChild(editBtn); body.appendChild(footer);
  showModal(t("studentProfileTitle"),body,"modal-box-wide");
}
...
function showAddStudentModal(presetClass=null) {
  ...
  footer.appendChild(el("button",{className:"btn-secondary",onClick:()=>overlay.remove()},t("cancel")));
  const ab=el("button",{className:"btn-primary"}); ab.style.borderRadius="8px"; ab.textContent=t("addStudent");
  ab.onclick=()=>{
...
function renderTimetable() {
  ...
  function buildSettingsView() {
    ...
    const addPBtn = el("button",{className:"btn-primary",style:{borderRadius:"8px",fontSize:"12px",padding:"6px 14px"}}, "+ " + t("addPeriod"));
...
function showAddPeriodModal(onDone) {
  const body = el("div",{});
  const {wrap:lw, input:li} = field(t("periodLabel"), {placeholder:"e.g. Period 6"});
  const {wrap:sw, input:si} = timeField(t("startTime"), "15:00");
  const {wrap:ew, input:ei} = timeField(t("endTime"),   "16:00");
  ...
  body.appendChild(tw);
  const footer = el("div",{className:"modal-footer"});
  const {overlay} = showModal(t("addNewPeriod"), body, "modal-box-narrow");
  footer.appendChild(el("button",{className:"btn-secondary",onClick:()=>overlay.remove()},t("cancel")));
  const addBtn = el("button",{className:"btn-primary",style:{borderRadius:"8px"}},t("addPeriodAction"));
  addBtn.onclick = () => {
...
function showEditPeriodModal(period, onDone) {
  const body = el("div",{});
  const {wrap:lw, input:li} = field(t("periodLabel"), {placeholder:"e.g. Period 3", value:period.label});
  const {wrap:sw, input:si} = timeField(t("startTime"), period.start);
  const {wrap:ew, input:ei} = timeField(t("endTime"),   period.end);
  const {wrap:tw, input:ti} = field(t("type"), {type:"select", options:[
    {value:"lesson",label:t("lesson")},{value:"break",label:t("break")},{value:"lunch",label:t("lunch")}
  ]});
...
function renderClasses() {
  ...
  const afb=el("button",{className:"btn-primary"},"+ " + t("addForm")); afb.onclick=()=>showAddFormModal(); afc.appendChild(afb); sg.appendChild(afc); page.appendChild(sg);
...
function renderClassDetail(className) {
  ...
  const card=el("div",{className:"card"}); const th2=el("div",{className:"table-header"}); th2.appendChild(el("span",{},t("studentsInThisClass"))); card.appendChild(th2);
